...
public class ESUnJail extends ECommand<EverSanctions> {
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> args) throws CommandException {
		...
		if (args.size() == 2) {
			...
			// Le joueur existe
			if (user.isPresent()){
				return this.commandUnJail(source, user.get(), args.get(1));
	...
	private CompletableFuture<Boolean> commandUnJail(final CommandSource staff, EUser user, String reason_string) {
