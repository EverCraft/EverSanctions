...
public class ESUnMute extends ECommand<EverSanctions> {
	
	public ESUnMute(final EverSanctions plugin) {
        super(plugin, "unmute");
    }
	...
	public CompletableFuture<Boolean> execute(final CommandSource source, final List<String> args) throws CommandException {
		...
		if (args.size() == 2) {
			...
			// Le joueur existe
			if (user.isPresent()){
				return this.commandUnMute(source, user.get(), args.get(1));
	...
	private CompletableFuture<Boolean> commandUnMute(final CommandSource staff, EUser user, String reason_string) {
